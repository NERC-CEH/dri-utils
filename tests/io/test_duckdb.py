...
class TestDuckDBFileReader:
    def test_initialization(self) -> None:
        """Test that the class can be initialized"""
        reader = DuckDBFileReader()

        assert isinstance(reader._connection, DuckDBPyConnection)

    @patch("driutils.io.duckdb.DuckDBFileReader.close")
    def test_context_manager_is_functional(self, mock_close_connection: MagicMock) -> None:
        """Should be able to use context manager to auto-close file connection"""

        with DuckDBFileReader() as con:
            assert isinstance(con._connection, DuckDBPyConnection)

        mock_close_connection.assert_called_once()

    @patch("driutils.io.duckdb.DuckDBFileReader.close")
    def test_connection_closed_on_delete(self, mock_close_connection: MagicMock) -> None:
...
class TestDuckDBS3Reader:
    @pytest.mark.parametrize("value", ["a", 1, "cutom_endpoint"])
    def test_value_error_if_invalid_auth_option(self, value: str | int) -> None:
    ...
    def test_upper_or_lowercase_option_accepted(self, mock_authenticate: MagicMock, value: str) -> None:
    ...
    def test_init_auto_authentication(self, mock_reader: MagicMock) -> None:
    ...
    def test_init_sts_authentication(self, mock_reader: MagicMock) -> None:
        """Tests that the reader can use the 'sts' auth option"""
        DuckDBS3Reader("sts")
        mock_reader.assert_called_once()

    @pytest.mark.parametrize("url, ssl", [["https://s3-a-real-endpoint", True], ["http://localhost:8080", False]])
    @patch.object(DuckDBS3Reader, "_custom_endpoint_auth", wraps=DuckDBS3Reader._custom_endpoint_auth, autospec=True)
    def test_init_custom_endpoint_authentication_https(self, mock_reader: MagicMock, url: str, ssl: bool) -> None:
        """Tests that the reader can authenticate to a custom endpoint
        with https protocol"""
        reader = DuckDBS3Reader("custom_endpoint", url, ssl)
        mock_reader.assert_called_once_with(reader, url, ssl)
